...
from gwosc import (
    api as gwosc_api,
    datasets as gwosc_datasets,
    locate as gwosc_locate,
    timeline as gwosc_timeline,
    urls as gwosc_urls,
    utils as gwosc_utils,
)
...
def gw150914_urls(scope='module'):
    return gwosc_api.fetch_event_json('GW150914')['strain']


@pytest.fixture
def gw170817_urls(scope='module'):
    return gwosc_api.fetch_event_json('GW170817')['strain']
...
def test_api_fetch_dataset_json():
    ...
    out = gwosc_api.fetch_dataset_json(start, end)
...
def test_api_fetch_event_json():
    event = 'GW150914'
    out = gwosc_api.fetch_event_json(event)
...
def test_api_fetch_run_json():
    ...
    out = gwosc_api.fetch_run_json(run, detector, start, end)
...
def test_find_datasets():
    ...
    with pytest.raises(ValueError):
        gwosc_datasets.find_datasets(type='badtype')


def test_event_gps():
    assert gwosc_datasets.event_gps('GW170817') == 1187008882.43
    with pytest.raises(ValueError) as exc:
        gwosc_datasets.event_gps('GW123456')
    assert str(exc.value) == 'no event dataset found for \'GW123456\''


def test_event_at_gps():
    assert gwosc_datasets.event_at_gps(1187008882) == 'GW170817'
    with pytest.raises(ValueError) as exc:
        gwosc_datasets.event_at_gps(1187008882, tol=.1)
    assert str(exc.value) == 'no event found within 0.1 seconds of 1187008882'


def test_run_segment():
    assert gwosc_datasets.run_segment('O1') == (1126051217, 1137254417)
    with pytest.raises(ValueError) as exc:
        gwosc_datasets.run_segment('S7')
...
def test_get_urls():
    ...
    span = (start, end)
    urls = gwosc_locate.get_urls(detector, start, end)
    for url in urls:
        assert os.path.basename(url).startswith(
            '{}-{}'.format(detector[0], detector))
        assert gwosc_utils.segments_overlap(
            gwosc_utils.url_segment(url), span)
...
def test_get_event_urls(gw150914_urls):
    ...
    event = 'GW150914'
    urls = gwosc_locate.get_event_urls(event)
    for url in urls:
    ...
    urls = gwosc_locate.get_event_urls(event, version=1)
    for url in urls:
        assert '_V1-' in url
...
def test_timeline_get_segments(flag, start, end, result):
    assert gwosc_timeline.get_segments(flag, start, end) == result
...
def test_url_segment():
    ...
    assert gwosc_utils.url_segment(url) == (123, 579)
...
def test_url_overlaps_segment(url, segment, result):
    assert gwosc_utils.url_overlaps_segment(url, segment) is result
...
def test_full_coverage(gw150914_urls, segment, result):
    urls = [u['url'] for u in gw150914_urls]
    assert gwosc_utils.full_coverage(urls, segment) is result
...
def test_segments_overlap(seg1, seg2, result):
    assert gwosc_utils.segments_overlap(seg1, seg2) is result
